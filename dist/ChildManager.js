...
export class ChildManager {
    ...
    spawnCommand(opts) {
        ...
        const child = spawn(opts.cmd, args, spawnOpts);
        if (typeof child.pid === 'number')
            this.processes.set(child.pid, child);
        else
            this.pidless.add(child);
     ...
     */
    spawnSyncCommand(cmd, args, opts) {
        const maxBuffer = opts?.maxBufferBytes ?? this.DEFAULT_MAX_BUFFER;
     ...
     */
    killByCommand(cmdName, signal = 'SIGTERM') {
